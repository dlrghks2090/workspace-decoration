...
> **아래 출력은 보고서 작성에 착수한 시점(최신 커밋 `34fe843`)의 기록이다.** 이후 실습을 진행하며 커밋이 쌓였고 17-5에서 원격을 SSH로 전환했으므로, 지금 다시 실행하면 이력이 더 길고 URL도 `git@github.com:` 형태로 나온다. 연동이 확인됐다는 사실 자체는 달라지지 않는다.
...
또한 10번에서 만든 `my-app` 과 15-2에서 만든 `my-app2` 가 같은 `my-web:v1` 이미지로 동시에 떠 있었다. 8번의 `docker ps -a` 출력에서 두 컨테이너의 `IMAGE` 열이 모두 `my-web:v1` 인 것이 그 증거다. **이미지 1개 : 컨테이너 N개** 관계가 성립한다.
...
- **결과**: 공개키 원문을 붙여넣자 오류 없이 등록됐고, 이어진 인증도 성공했다 (`ssh -T` 출력은 17-5 참고).
...
| 폐기 방법 | 토큰 재발급 | GitHub에서 해당 공개키만 삭제 |
  ...
  **현재 키의 passphrase 미설정 — 인지한 상태에서의 선택**

  이번에 생성한 키에는 passphrase를 걸지 않았다. 따라서 위 표의 "개인키에 passphrase를 걸면 파일만으로는 사용 불가"라는 이점은 **지금 이 키에는 적용되지 않는다.** 현재 상태를 명확히 하면 다음과 같다.

      $ ssh-keygen -y -P "" -f ~/.ssh/id_ed25519
      (빈 passphrase로 열림 → 개인키가 평문으로 저장되어 있음)

| 항목 | 현재 상태 |
| :--- | :--- |
| 개인키 저장 형태 | 평문 (암호화되지 않음) |
...
| 보완하고 있는 것 | 파일 권한 `600`(소유자 전용), 개인 노트북 단독 사용, 저장소·문서에 개인키 미기재 |
  ...
  **필요해지면**: 키를 다시 만들 필요 없이 아래 한 줄로 추가할 수 있다. 공개키는 변하지 않으므로 GitHub 재등록도 필요 없다.
...
- **차단 동작 검증**
  ...
  > 위 `git status` 는 신규 파일 5개를 **커밋하기 직전** 시점의 출력이다. 지금은 모두 커밋돼 목록이 비어 있지만, `.env` 가 어느 쪽에도 나타나지 않는다는 점은 동일하다. 시점과 무관하게 재현되는 근거는 위의 `git check-ignore` 쪽이다.

### 체크리스트
- [x] 로그 내 이메일·홈 경로 등 개인 식별 정보 마스킹 처리 완료
- [x] `.gitignore` 파일 생성 및 `.env`·시스템 파일 업로드 차단 검증 완료
- [x] 실제 설정 값(`.env`)과 예시(`.env.example`) 분리 완료
- [x] SSH 개인키를 저장소·문서 어디에도 기재하지 않음 (공개키도 지문만 기록)
- [x] SSH 개인키 파일 권한이 `600`(소유자 전용)인지 확인 완료
- [x] passphrase 미설정 상태와 그 위험을 17-5에 명시 (인지한 상태에서의 선택)
