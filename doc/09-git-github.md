...
보고서 12번이 Git 연동을 증명할 때 쓴 네 명령이다. 옵션에 따라 출력이 크게 달라지므로 정리해 둔다.

### `git status` — `--short` 와 `-sb`

기본형은 안내 문구까지 붙어 10줄 안팎이 된다. 사람이 읽기엔 친절하지만 문서에 붙이기엔 길다.

```bash
git status              # 10줄 (브랜치 안내 + 사용법 힌트 포함)
```

`--short`(=`-s`)를 주면 변경 파일만 두 글자 코드와 함께 나온다.

```bash
git status --short
#  M doc/03-docker-concepts.md
```

앞 두 칸이 **스테이징 영역과 작업 트리의 상태**를 각각 나타낸다. 이게 3개 영역 개념과 직접 연결된다.
...
`-sb` 는 `-s` 에 **브랜치 줄(`-b`)** 을 더한 것이다.

```bash
git status -sb
# ## main...origin/main          ← -b 가 붙여주는 줄
#  M doc/03-docker-concepts.md
```

`## main...origin/main` 은 로컬과 원격이 같은 지점이라는 뜻이다. 차이가 나면 `[ahead 1]`, `[behind 2]` 가 붙는다. **푸시 전에 이 한 줄만 봐도 밀린 커밋이 있는지 알 수 있어서** 보고서 17-5의 SSH 검증에 썼다.

### `git log --oneline`

기본형은 커밋 하나가 헤더 4줄 + 메시지 전체다. 커밋 메시지를 길게 쓰는 이 저장소에서는 한 커밋이 30줄을 넘기도 한다.

```bash
git log --oneline -3
# e804333 Docs: 데몬 확인 방법 서술을 정정하고 --version / version 차이를 명시
# cdf0586 Docs: 학습 자료 전수 검토 - 날조된 출력 5건과 umask 설명 오류 정정
# 4fe491c Docs: "호스트 커널 공유"와 macOS VM 설명의 충돌을 실측으로 정리
```

**커밋 하나가 한 줄**이 되고, 해시는 앞 7자리로 줄어든다. 이력의 흐름을 볼 때 쓴다.

### `git branch -vv` — `v` 를 두 번 쓰는 이유

세 형태를 나란히 놓으면 차이가 분명하다.
...
git branch -v
# * main e804333 Docs: 데몬 확인 방법 서술을 정정하고 ...

git branch -vv
# * main e804333 [origin/main] Docs: 데몬 확인 방법 서술을 정정하고 ...
#                └─────┬─────┘
#                      └── -v 를 한 번 더 줘야 나온다
```
...
| (없음) | 브랜치 이름과 현재 위치(`*`) |
| `-v` | + 최신 커밋 해시와 메시지 |
| `-vv` | + **업스트림 추적 정보** `[origin/main]` |

보고서 12번이 `-vv` 를 쓴 이유가 이것이다. 루브릭이 "GitHub **연동**이 확인되는가"를 묻는데, `[origin/main]` 이 바로 **로컬 브랜치가 원격을 추적하도록 연결됐다**는 증거다. `-v` 까지만 쓰면 이 정보가 빠진다.

### `git config --list`

설정을 전부 나열한다. 시스템·전역·로컬 설정이 합쳐진 최종 결과다.
...
너무 많아서 보고서에서는 `grep -E "^user\.|^remote\."` 로 필요한 줄만 걸렀다.

| 옵션 | 어느 범위를 보나 |
| :--- | :--- |
| `--list` | 전부 합친 최종 값 |
| `--local` | 이 저장소의 `.git/config` |
| `--global` | 계정 전체 `~/.gitconfig` |
| `--show-origin` | 각 값이 **어느 파일에서 왔는지**까지 표시 |

같은 키가 여러 범위에 있으면 **좁은 범위가 이긴다**(local > global > system). 어느 파일이 이겼는지 알고 싶을 때 `--show-origin` 이 유용하다.
