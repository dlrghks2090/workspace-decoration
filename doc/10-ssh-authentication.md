...
같은 명령인데 옵션에 따라 **키를 만들기도 하고, 읽기만 하기도 한다.** 섞이면 위험하므로 따로 정리한다.

### 보고서에서 쓴 것

| 명령 | 하는 일 |
| :--- | :--- |
| `ssh-keygen -t ed25519 -C "메일"` | 키 **생성** |
| `ssh-keygen -lf ~/.ssh/id_ed25519.pub` | 지문 **조회** |
| `ssh-keygen -y -P "" -f ~/.ssh/id_ed25519` | passphrase 여부 **확인** |
...
| 옵션 | 이름 | 역할 |
| :--- | :--- | :--- |
| `-t` | type | 키 알고리즘 (`ed25519`, `rsa`, `ecdsa`) |
| `-C` | comment | 공개키 끝에 붙는 **메모** |

`-C` 가 흔히 오해받는다. **이메일을 적지만 인증에 쓰이지 않는다.** 그냥 주석이라 아무 문자열이나 넣어도 되고, 지워도 키는 정상 동작한다. 목적은 GitHub에 키를 여러 개 등록했을 때 **어느 기기의 키인지 구분**하는 것이다. 그래서 `-C "macbook-2026"` 처럼 기기명을 넣는 방식도 많이 쓴다.

`-t ed25519` 를 쓴 이유는 [1장](#1-공개키-암호-방식의-원리)에서 다뤘다. RSA보다 키가 짧고 빠르면서 안전성은 동등 이상이다.
...
> **`-l` 을 빼면 위험하다.** `-f` 만 주면 ssh-keygen은 "그 경로에 키를 **생성**하라"는 뜻으로 받아들인다.
>
> ```bash
> ssh-keygen -f ~/.ssh/id_ed25519.pub
> # Generating public/private ed25519 key pair.
> # /Users/****/.ssh/id_ed25519.pub already exists.
> # Overwrite (y/n)?          ← 여기서 y 를 누르면 기존 키를 잃는다
> ```
>
> 조회하려던 명령이 덮어쓰기 확인을 물어오는 것이다. **`-l` 과 `-f` 는 반드시 함께 쓴다.**

지문 해시 알고리즘은 `-E` 로 바꿀 수 있다. GitHub 화면은 SHA256을 쓰지만, 오래된 문서에는 MD5 형식이 남아 있다.

```bash
ssh-keygen -lf ~/.ssh/id_ed25519.pub -E md5
# 256 MD5:88:d9:f5:46:b5:61:f0:e6:0e:db:7c:2f:06:c8:a6:01 ****@gmail.com (ED25519)
```
...
# ssh-ed25519 AAAAC3NzaC1lZDI1NT... (공개키 원문)
```

| 옵션 | 이름 | 역할 |
| :--- | :--- | :--- |
| `-y` | yield | 개인키를 읽어 **공개키를 다시 만들어** 출력 |
| `-P` | passphrase | passphrase를 **인자로** 전달 (`""` = 빈 값) |

`-y` 가 성립하는 이유는 **공개키가 개인키에서 계산으로 유도되기 때문**이다. 반대는 불가능하다. 그래서 `.pub` 파일을 실수로 지워도 개인키만 있으면 복구된다.

보고서 17-5는 이걸 **passphrase 유무를 판정**하는 데 썼다. `-P ""` 로 "빈 passphrase"를 주었는데 키가 열렸다면, 그 키에는 passphrase가 없다는 뜻이다. 걸려 있었다면 여기서 실패한다.

> **`-P` 를 일상적으로 쓰지 말 것.** passphrase가 셸 히스토리와 프로세스 목록에 평문으로 남는다. 위처럼 빈 문자열을 확인하는 용도가 아니면 대화형 입력을 쓴다.
...
`-p` 는 소문자이고 `-P`(대문자)와 **전혀 다르다.**

| | 역할 |
| :--- | :--- |
| `-p` (소문자) | passphrase를 **바꾼다** (대화형으로 묻는다) |
| `-P` (대문자) | passphrase를 **인자로 넘긴다** |

키 자체는 그대로 두고 암호화만 다시 하므로 **공개키가 변하지 않는다.** 그래서 나중에 passphrase를 걸어도 GitHub 재등록이 필요 없다 (보고서 17-5의 "필요해지면" 항목).
...
`-T` 는 **의사 터미널(TTY) 할당을 하지 않겠다**는 뜻이다. `T`가 terminal이고, 대문자가 "끄기"다.
...
보통 `ssh 서버` 로 접속하면 원격 셸을 쓰라고 터미널을 붙여준다. 그런데 **GitHub은 셸 접속을 제공하지 않으므로** 터미널을 요청하면 경고가 뜬다. `-T` 는 "셸은 필요 없고 인증만 해보겠다"는 선언이다.

### 종료 코드가 1이어도 정상이다

```bash
ssh -T git@github.com; echo $?
# Hi dlrghks2090! You've successfully authenticated, ...
# 1
```

인증에 성공해도 GitHub이 곧바로 세션을 끊기 때문에 종료 코드는 `1` 이다. **판단 기준은 종료 코드가 아니라 `successfully authenticated` 문구다.** 스크립트에서 `if ssh -T ...` 로 분기하면 항상 실패로 잡힌다.
